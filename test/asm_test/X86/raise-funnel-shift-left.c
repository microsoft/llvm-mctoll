...
// CHECK: MRI : SHLD r16, r16, 8
// CHECK-NEXT: Shift 0x1234 by 8
// CHECK-NEXT: Funnel in 0x5678
// CHECK-NEXT: Shifted Val  = 0x3456
// CHECK-NEXT: Funnel in Val = 0x5678
// CHECK-NEXT: MRI : SHLD r32, r32, 16
// CHECK-NEXT: Shift 0x12345678 by 16
// CHECK-NEXT: Funnel in 0x9abcdef0
// CHECK-NEXT: Shifted Val  = 0x56789abc
// CHECK-NEXT: Funnel in Val = 0x9abcdef0
// CHECK-NEXT: MRI : SHLD r64, r64, 24
// CHECK-NEXT: Shift 0x123456789abcdef0 by 24
// CHECK-NEXT: Funnel in 0xdeadbeefdeadbeef
// CHECK-NEXT: Shifted Val  = 0x789abcdef0deadbe
// CHECK-NEXT: Funnel in Val = 0xdeadbeefdeadbeef
// CHECK-NEXT: MRC : SHLD r16, r16, CL
// CHECK-NEXT: Shift 0x1234 by 8
// CHECK-NEXT: Funnel in 0x5678
// CHECK-NEXT: 16-bit Shifted Val  = 0x3456
// CHECK-NEXT: Funnel in Val = 0x5678
// CHECK-NEXT: MRC : SHLD r32, r32, CL
// CHECK-NEXT: Shift 0x12345678 by 8
// CHECK-NEXT: Funnel in 0x9abcdef0
// CHECK-NEXT: Shifted Val  = 0x3456789a
// CHECK-NEXT: Funnel in Val = 0x9abcdef0
// CHECK-NEXT: MRC : SHLD r64, r64, CL
...
funnel_shift_left_8_16rri(uint16_t value, uint16_t in) {
...
funnel_shift_left_16_32rri(uint32_t value, uint32_t in) {
...
funnel_shift_left_24_64rri(uint64_t value, uint64_t in) {
...
uint16_t __attribute__((noinline))
funnel_shift_left_cl_16rri(uint16_t value, uint16_t in, uint8_t count) {
  uint16_t in_res, shifted_val;
...
uint32_t __attribute__((noinline))
funnel_shift_left_cl_32rri(uint32_t value, uint32_t in, uint8_t count) {
  uint32_t in_res, shifted_val;
...
uint64_t __attribute__((noinline))
funnel_shift_left_cl_64rri(uint64_t value, uint64_t in, uint8_t count) {
  uint64_t in_res, shifted_val;
...
int main() {
  printf("MRI : SHLD r16, r16, 8\n");
  uint16_t s16 = funnel_shift_left_8_16rri(0x1234, 0x5678);
  printf("MRI : SHLD r32, r32, 16\n");
  uint32_t s32 = funnel_shift_left_16_32rri(0x12345678, 0x9abcdef0);
  printf("MRI : SHLD r64, r64, 24\n");
  uint64_t s64 =
      funnel_shift_left_24_64rri(0x123456789abcdef0, 0xdeadbeefdeadbeef);
  printf("MRC : SHLD r16, r16, CL\n");
  uint16_t s16_cl = funnel_shift_left_cl_16rri(0x1234, 0x5678, 8);
  printf("MRC : SHLD r32, r32, CL\n");
  uint32_t s32_cl = funnel_shift_left_cl_32rri(0x12345678, 0x9abcdef0, 8);
  printf("MRC : SHLD r64, r64, CL\n");
  uint64_t s64_cl =
      funnel_shift_left_cl_64rri(0x123456789abcdef0, 0xdeadbeefdeadbeef, 8);
  return 0;
}
